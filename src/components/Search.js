...
export class Search extends React.Component {
  ...
  searchSent (query, e) {
    ...
    for (let j = 0; j < sourceArray.length; j++) {
      ...
      let url = `https://newsapi.org/v2/everything?apiKey=a8bbe0f664d741539f4eeb966fa99339&q=${query}&sources=${
        sourceArray[j]
      ...
      fetch(req)
        ...
        .then(response => {
          ...
          this.props.dispatch(
            populateState(siteArray[j], titleAndUrl, filteredArray)
          )
  ...
  render () {
    ...
    return (
      <form className={formClass} onSubmit={e => {
                                        this.searchSent(this.textInput.value, e)
                                      }}>
        <input
          value='Putin'
          autoFocus
